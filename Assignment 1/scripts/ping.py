...
hosts = ['google.com','github.com','bitbucket.org','slack.com','stackexchange.com']	
sizes = ["64", "128", "256", "512", "1024", "2048"]
for host in hosts:
    query = {'q':host}
    response = requests.request("GET", trace_url, params=query)
    with open('ping_traceroutes4.txt','a') as tr_record:
    ...
    #     with open('ping_results3.csv', 'a') as csvfile:
    ...
    #                          'Time': time.strftime("%d %b %Y %H:%M:%S", time.localtime())
                             # })
print('Finished! Check the results in \'ping_results3.csv\' Enjoy!!')
